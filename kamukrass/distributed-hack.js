...
let weakenScriptName = "weaken.js";
let growScriptName = "grow.js";
let hackScriptName = "hack.js";
let shareScriptName = "share.js";
const shareScriptRam = 4;

// list of slave script files
let files = [weakenScriptName, growScriptName, hackScriptName];
...
export async function main(ns) {
    // Resolve worker paths relative to this script, so the whole folder can be moved.
    const ownPath = ns.getScriptName();
    const lastSlash = ownPath.lastIndexOf("/");
    let folder = lastSlash >= 0 ? ownPath.slice(0, lastSlash + 1) : "";
    if (folder && !folder.startsWith("/")) folder = `/${folder}`;

    weakenScriptName = `${folder}weaken.js`;
...
function manageAndHack(ns, freeRams, servers, targets, growStocks, hackStocks) {
    ...
    for (let target of targets) {
        ...
        if (secDiff < 0.5) {
            ...
            // All servers have at least one core. Use the minimum so distributed
            // grow jobs never assume the extra bonus from home's additional cores.
...
function getHackable(ns, servers) {
    ...
    var sortedServers = [...servers.values()].filter(server => {
        const info = ns.getServer(server);
        return !info.purchasedByPlayer
            && (info.moneyMax ?? 0) > 100000
            && (info.requiredHackingSkill ?? Infinity) <= hackingLevel
            && (info.serverGrowth ?? 0) > 1
            && server != "n00dles";
...
async function scanAndNuke(ns) {
    ...
    for (let server of servers) {
        if (ns.hasRootAccess(server)) {
            ...
            // In v3, nuke and the port openers return false instead of throwing.
            if (ns.nuke(server)) {
                accessibleServers.add(server);
            }
        }
