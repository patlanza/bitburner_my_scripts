...
export async function main(ns) {
    ns.disableLog("ALL");
    const logPortNumber = Number(ns.args[0] ?? 0);
    const logPort = Number.isInteger(logPortNumber) && logPortNumber >= 1 && logPortNumber <= 20
        ? ns.getPortHandle(logPortNumber)
        : null;
    ...
    //while (true) {    
        ...
        const contracts = servers.flatMap((server) => {
            const onServer = ns.ls(server, ".cct").map((contract) => {
                ...
                if (didSolve === null) {
                    return `${server} - ${contract} - ${type} - OMITIDO (sin algoritmo)`;
                }
        ...
        if (contracts.length === 0) {
            contracts.push("No se encontraron contratos.");
        }
        contracts.forEach((contract) => {
            ns.print(contract);
            logPort?.write(contract);
        });
...
function solve(type, data, server, contract, ns) {
    ...
    switch (type) {
        ...
        case "Sanitize Parentheses in Expression":
            ...
            break;
        default:
            // Un tipo desconocido no debe consumir ningún intento.
            return null;
    }
